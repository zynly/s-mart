...
class CashierSessionService
{
    public function open(User $user, CashAccount $cashAccount, int $openingCash): CashierSession
    {
        return DB::transaction(function () use ($user, $cashAccount, $openingCash) {
            ...
            if ($accountOpen !== null) {
                $userOpenName = $accountOpen->user?->name ?? 'kasir lain';
                throw new DomainException("Laci {$account->name} sedang dibuka oleh {$userOpenName}. Silakan pilih laci kasir lain.");
            }
