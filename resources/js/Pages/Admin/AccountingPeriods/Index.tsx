...
export default function Index({ tab, periods }: AccountingPeriodsIndexProps) {
  ...
  const isAllSelected = useMemo(() => {
    ...
    return periods.every((p) => selectedIds.includes(p.id))
  }, [periods, selectedIds])
    ...
    <div className="flex flex-col gap-4">
          ...
          <thead className="bg-surface text-left text-content-muted">
            <tr>
              <th className="p-2 w-10 text-center">
                <Checkbox checked={isAllSelected} onCheckedChange={toggleSelectAll} aria-label="Pilih semua periode" />
              </th>
              <th className="p-2 w-12 text-center font-mono">No</th>
              <th className="p-2">Periode</th>
              <th className="p-2">Rentang</th>
              <th className="p-2">Status</th>
              <th className="p-2">Ditutup Oleh</th>
              <th className="p-2" />
            </tr>
          </thead>
          <tbody>
            {periods.map((period, index) => {
              ...
              return (
                <tr key={period.id} className={`border-t border-border ${isSelected ? 'bg-primary/5' : ''}`}>
                  <td className="p-2 text-center">
                    <Checkbox
                      checked={isSelected}
                      onCheckedChange={() => toggleSelectOne(period.id)}
                      aria-label={`Pilih periode ${period.month}/${period.year}`}
                    />
                  </td>
                  ...
                  <td className="p-2">
                    <Badge className={STATUS_BADGE[period.status]}>{STATUS_LABELS[period.status]}</Badge>
                  </td>
                  <td className="p-2">{period.closer?.name ?? '—'}</td>
                  <td className="p-2 text-right">
                    {period.status === 'closed' && (
                      <Button size="sm" variant="outline" onClick={() => setReopenTarget(period)}>
                        Buka Kembali
                      </Button>
                    )}
                  </td>
            ...
            {periods.length === 0 && (
              <tr>
                <td className="p-4 text-center text-content-muted" colSpan={7}>
                  Belum ada periode ditutup.
                </td>
              </tr>
            )}
          </tbody>
        ...
        <DialogContent>
          ...
          <form onSubmit={submitClose} className="flex flex-col gap-4 px-1">
            <p className="text-sm text-content-muted">
              Jurnal penutup akan dibuat otomatis (pendapatan &amp; beban dipindah ke Laba Ditahan). Sesudah ditutup,
              jurnal baru tidak bisa ditambahkan ke periode ini kecuali dibuka kembali.
            </p>
      ...
      <AlertDialog open={reopenTarget !== null} onOpenChange={(open) => !open && setReopenTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Buka kembali periode {reopenTarget && `${MONTH_NAMES[reopenTarget.month - 1]} ${reopenTarget.year}`}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              Jurnal baru akan bisa ditambahkan lagi ke periode ini. Jurnal penutup yang sudah dibuat TIDAK otomatis
              dibalik — pastikan ini memang disengaja.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
