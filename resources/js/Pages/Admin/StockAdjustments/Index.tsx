                ...
                {lines.map((line) => (
                  <div key={line.key} className="flex flex-col gap-2 p-2">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium">{line.product_name} — {line.qty}</span>
                    ...
                    {form.data.type === 'decrease' && (
                      <Select value={line.stock_layer_id} onValueChange={(v) => updateLine(line.key, { stock_layer_id: v })}>
                        <SelectTrigger className="w-full"><SelectValue placeholder="Pilih layer stok" /></SelectTrigger>
                        <SelectContent>
